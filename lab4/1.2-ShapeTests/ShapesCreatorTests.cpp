#include "stdafx.h"
#include "../1.2-Shape/ShapesCreator.h"

using namespace std;

struct ShapesCreatorFixture
{
	CShapesCreator shapesCreator;

	ShapesCreatorFixture()
		: shapesCreator()
	{
	}
};

BOOST_FIXTURE_TEST_SUITE(ShapesCreator, ShapesCreatorFixture)

	BOOST_AUTO_TEST_CASE(can_handle_an_unknown_command)
	{
		string input = "LineSegmentSuper 12.5 12.5 -12.5 -12.5 ff0000";
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input), exception);
	}
	BOOST_AUTO_TEST_CASE(can_create_line_segment)
	{
		string input = "LineSegment 12.5 12.5 -12.5 -12.5 ff0000";
		BOOST_CHECK_NO_THROW(shapesCreator.CreateShapesFromString(input));
	}
	BOOST_AUTO_TEST_CASE(can_recognize_invalid_string_when_creating_line_segment)
	{
		...
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input2), exception);

		string input3 = "LineSegment 12.5 12.5 -12.5 -12.5 -ff0000";
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input3), exception);

		string input4 = "LineSegment 12.5 12.5 -12.5 -12.5 ff000m";
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input4), exception);

		string input5 = "LineSegment 12.5 12.5 -12.5 -12.5 ff00000";
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input5), exception);

		string input6 = "LineSegment 12.5 12.5 -12.5 ff0000";
		BOOST_CHECK_THROW(shapesCreator.CreateShapesFromString(input6), exception);
	}
